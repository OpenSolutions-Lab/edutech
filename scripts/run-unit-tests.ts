...
import { calcularScoreEvasao, DadosEscolaEvasao } from '../src/lib/ai/evasao-model';
import { calcularProjecaoDemandaAdensamento } from '../src/lib/ai/demanda-adensamento-model';
import { analisarGapsEmpregabilidadeTecnica } from '../src/lib/ai/empregabilidade-tecnico-model';
import { calcularIDSBairro } from '../src/lib/ai/ids-vulnerabilidade-model';

console.log('🧪 Iniciando Execução dos Testes Unitários dos Algoritmos Intersetoriais...');

try {
  // Teste 1: Evasão com Alerta VEL (Vulnerabilidade Econômica Local)
  const dadosEvasao: DadosEscolaEvasao = {
    id: 'esc-1',
  ...
  const evasaoResult = calcularScoreEvasao(dadosEvasao);
  ...
  // Teste 4: IDS Territorial IPP (Alta vulnerabilidade)
  const idsResultAlta = calcularIDSBairro(110, 0.44, 0.39, 0.41);
  assert.strictEqual(idsResultAlta.faixa_vulnerabilidade, 'Alta', 'Score ~0.415 deve resultar em vulnerabilidade Alta');

  // Teste 5: IDS Territorial IPP (Extrema vulnerabilidade)
  const idsResultExtrema = calcularIDSBairro(82, 0.30, 0.25, 0.30);
  assert.strictEqual(idsResultExtrema.faixa_vulnerabilidade, 'Extrema', 'Score <0.40 deve resultar em vulnerabilidade Extrema');
  console.log('✅ Teste 4: Cálculo do IDS Intersetorial IPP/SME/SMDEIS — PASSOU!');

  console.log('🎉 TODOS OS TESTES UNITÁRIOS DOS ALGORITMOS PASSARAM COM SUCESSO!');
} catch (e) {
